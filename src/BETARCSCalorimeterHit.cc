...
G4Allocator<BETARCSCalorimeterHit> BETARCSCalorimeterHitAllocator;
   ...
   /*
     ...
     if(pVVisManager && (fDepositedEnergy>0.)) {
       ...
       // HandsOn5: Draw a box with depth propotional to the energy deposition
           G4double scale = 1000.;//BETAPrimaryGeneratorAction::Gun()->GetParticleEnergy();
       ...
       G4Box box("MyBox", 3.8*cm/2.0, 3.8*cm/2.0, depth);
...
const std::map<G4String,G4AttDef>* BETARCSCalorimeterHit::GetAttDefs() const
{
   G4bool isNew;
   std::map<G4String,G4AttDef>* store
   = G4AttDefStore::GetInstance ( "BETARCSCalorimeterHit",isNew );
...
std::vector<G4AttValue>* BETARCSCalorimeterHit::CreateAttValues() const
{
   std::vector<G4AttValue>* values = new std::vector<G4AttValue>;

   values->push_back ( G4AttValue ( "HitType", "RCSCalorimeterHit", "" ) );
...
void BETARCSCalorimeterHit::Print()
{
